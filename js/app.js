...
(function () {
  ...
  const 相簿 = (function () {
    ...
    function 開() {
      return new Promise((res) => {
        if (db) return res(db);
        const r = indexedDB.open("我會注意安全照片", 1);
  ...
  const 點名 = (function () {
    ...
    async function 匯出() {
      ...
      const 資料 = { 格式: "我會注意安全點名名單", 版本: 1, 匯出時間: new Date().toISOString(), 學生: [] };
      for (const s of 名單) {
  ...
  const 教學 = (function () {
    ...
    /* 🔥 2026-08-04：開場只有按「← 回重點清單」才會收乾淨。用**導覽鈕**離開時，
       開場區會帶著 .顯示 留著，接著點重點就變成開場區與重點內容
       **同時是 flex 並排堆疊**，把重點內容的按鈕擠到畫面外。
       視窗 720px 高時剛好還塞得下（所以線上的站都沒被發現），
       768px 就會超出（平面圖形實測按鈕底部落在 811px）。
    ...
    function 畫教學頁() {
      ...
      $("#重點內容").innerHTML = `
        ...
        <div class="按鈕列 置中">
          ...
          ${目前重點.短片 ? `<button class="次要鈕" id="鈕看短片">🎬 看教學短片</button>` : ""}
  ...
  /* ================================================================
     ...
     所以四個重點、五個重點、或某個重點沒有短片，都不必改這段程式。
     ================================================================ */
  function 建影片清單() {
    const 播放器 = $("#影片播放器");
    const 清單 = $("#影片清單");
    const 項目 = [{ 圖示: "\uD83C\uDFAC", 標題: "教學影片（完整版）", 檔: UNIT.資源.影片 }].concat(
      (UNIT.重點 || []).filter((r) => r.短片)
        .map((r) => ({ 圖示: r.圖示, 標題: r.短片.標題, 檔: r.短片.檔 })));
